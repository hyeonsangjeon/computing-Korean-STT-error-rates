...
`korean-v1`은 CER·WER·CRR을 바꾸거나 대체하지 않고 모델 차이를 설명하기
위한 선택적 진단 묶음입니다. 기존 사용자의 결과가 흔들리지 않도록 기본값은
계속 `None`입니다.
...
문자열 규칙이므로 문맥에 따라 오분류할 수 있으며 `experimental` 상태를
결과 provenance에도 기록합니다. 띄어쓰기 진단은 어휘 오류가 섞인 문장을
억지로 띄어쓰기 오류로 분류하지 않고 `skipped_lexical_items`로 제외합니다.

`keywords` 또는 `entities`를 함께 전달하면 기존 평가 함수에서 계산한 keyword
recall/false positive와 entity F1/CER를 `metric_breakdowns`에도 연결합니다. 새로
계산하거나 지표 의미를 바꾸지 않습니다.
