...
사용자에게 배포되는 **Beta patch release**입니다. 기존 metric API는 충분한
사용 이력이 있지만 비교 계약은 실제 배포 이력이 없으므로 이번에 `0.1.0`과
Stable classifier를 선언하지 않습니다.

최소 `0.0.0.20`과 후속 `0.0.0.21` 두 patch release에서 아래 계약을 검증한
뒤 `0.1.0` 후보를 다시 판단합니다. patch 번호 자체가 안정성을 증명하는 것은
아니며 실제 호환성·오류 보고·배포 검증 결과를 함께 확인합니다.

## 기능 상태

| 기능 | 0.0.0.20 상태 | 호환성 약속 |
| --- | --- | --- |
