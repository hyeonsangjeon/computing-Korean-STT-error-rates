...
def _preprocess_cer_text(text: str, rm_punctuation: bool) -> str:
...
def get_cer(reference, transcription, rm_punctuation=True) -> MetricResult:
    ...
    refs = _preprocess_cer_text(reference, rm_punctuation)
    trans = _preprocess_cer_text(transcription, rm_punctuation)
    ...
    deletions = cer_d
    insertions = cer_i
    cer = _calculate_normalized_error_rate(substitutions, deletions, insertions, hits)
...
def get_wer(reference, transcription, rm_punctuation=True) -> MetricResult:
    ...
    wer = _calculate_normalized_error_rate(substitutions, deletions, insertions, hits)
    result = {'wer': wer, 'substitutions': substitutions, 'deletions': deletions, 'insertions': insertions}

    return result


def get_crr(reference, transcription, rm_punctuation=True) -> MetricResult:
    """
...
def _make_suffix_pattern(josa_list: Sequence[str], eomi_list: Optional[Sequence[str]] = None) -> str:
    suffixes = list(josa_list)
    if eomi_list:
        suffixes.extend(eomi_list)
    escaped_suffixes = [re.escape(suffix) for suffix in sorted(set(suffixes), key=lambda x: -len(x))]
    return "|".join(escaped_suffixes)


def make_keyword_pattern(keyword, josa_list, eomi_list=None) -> Pattern:
    """
    ...
    4. 정규식 특수문자를 포함한 키워드도 안전하게 매칭합니다.
    ...
    keyword_pattern = r'\s*'.join(re.escape(char) for char in keyword)
    ...
    if suffix_pattern:
        ...
        full_pattern = rf"(?<![{_KOREAN_TOKEN_CHAR}]){keyword_pattern}(?![{_KOREAN_TOKEN_CHAR}])"
...
def calculate_keyword_error_rate_with_pattern(reference_sentences, hypothesis_sentences, keywords, josa_list, eomi_list=None):
    ...
    if len(reference_sentences) != len(hypothesis_sentences):
        raise ValueError("reference_sentences and hypothesis_sentences must have the same length")
