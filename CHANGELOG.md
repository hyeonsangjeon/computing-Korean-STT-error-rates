...
### Fixed
- README 공식 계산식 이미지 아래에 `S` 치환, `D` 삭제, `I` 삽입, `N` 참조 단위 수, `C` 정답 일치 수 범례를 작은 한 줄로 복원.

### Compatibility
- 공개 API, 계산식, 기본값, 의존성과 comparison schema는 `0.0.0.21`에서 변경하지 않음.

### Tests
- 공식 이미지와 함께 `S·D·I·N·C` 범례가 README 및 PyPI 설명에 유지되는지 회귀 테스트 보강.
...
### Fixed
- 최신 README 개편에서 빠졌던 표준 CER·WER와 Nlptutti `normalized` 오류율 공식 이미지를 첫 화면에 복원.
- GitHub와 PyPI 패키지 설명에서 같은 공식 이미지를 표시하도록 기존 raw 이미지 경로를 다시 연결.

### Compatibility
- 공개 API, 계산식, 기본값, 의존성과 comparison schema는 `0.0.0.20`에서 변경하지 않음.

### Tests
- README가 공식 `FORMULA_CASE.png`를 계속 참조하고 원본 이미지 파일이 저장소에 존재하는지 회귀 테스트 추가.
