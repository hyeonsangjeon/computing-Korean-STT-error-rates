...
`parse_provider_transcript()`는 이미 저장한 공급자 JSON을 `nlptutti`의 공통
transcript 구조로 바꿉니다. 네트워크 요청, 인증, SDK 호출, 모델 실행은 하지
않으며 공급자와 스키마를 자동 감지하지도 않습니다.
...
| 실패 상태 | `NoMatch`, timeout, `Error`를 평가 결과로 위장하지 않고 예외 발생 |

`detailed`의 `NBest`, batch transcription, fast transcription은 서로 다른
계약이므로 이 버전에서 지원하지 않습니다. 공식 근거는 Microsoft의
...
OpenAI API의 transcription 응답과는 별도 계약입니다.
...
필드 근거는 고정 커밋 `5f86d1d86363843179951550570367b37c5d6f78`의
[`transcribe()` 반환 계약](https://github.com/openai/whisper/blob/5f86d1d86363843179951550570367b37c5d6f78/whisper/transcribe.py)입니다.
fixture는 공개 음성이나 공급자 응답을 복제하지 않고 이 문서의 필드 계약에
맞춰 직접 작성했습니다.

## 오류 정책

알 수 없는 provider/schema, JSON 문법 오류, 필수 필드 누락, 음수 또는 역전된
시간은 fail-closed 방식으로 거부합니다. 일반 JSON·SRT·TSV를 위한 기존
`parse_transcript()`의 동작과 strictness는 변경하지 않습니다.
