...
번 실행하고 실행 시간의 중앙값과 최대 peak memory를 비교합니다. 두 구현 모두
동적 계획법 표를 만든 뒤 실제 edit path까지 생성합니다. 한국어 문구를 반복한
8개 합성 문장 쌍을 사용하며 네트워크나 외부 데이터는 필요하지 않습니다.
실행시간 또는 peak memory 비율이 기존 구현의 `1.25`를 넘으면 GitHub Actions
형식의 경고를 출력합니다.
...
- 실행시간: 기존 `0.125819초`, 공용 커널 `0.131212초` (`1.043배`)
- peak memory: 기존 `143,888바이트`, 공용 커널 `143,872바이트` (`1.000배`)
